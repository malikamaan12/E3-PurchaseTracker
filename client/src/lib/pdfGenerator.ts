  ...
  try {
    ...
    if (request?.pdfSettings?.headerHeight) {
      ...
      headerHeight = Math.min(
        Math.max(request.pdfSettings.headerHeight / 2, 10),
        100,
      );
    }
    ...
    if (request?.pdfSettings) {
      ...
      settings = {
        ...
        headerSubtitle:
          request.pdfSettings.headerSubtitle || settings.headerSubtitle,
      ...
      try {
        ...
        if (response.ok) {
          ...
          settings = {
            ...
            headerSubtitle:
              apiSettings.headerSubtitle || settings.headerSubtitle,
    ...
    if (settings.showHeaderText === true) {
      ...
      const headerTextX = settings.showLogo
        ? pageWidth / 2 + 10
        : pageWidth / 2;
      ...
      doc.text(settings.headerTitle, headerTextX, headerTextY, {
        align: "center",
      });
      ...
      doc.text(settings.headerSubtitle, headerTextX, headerTextY + 6, {
        align: "center",
      });
      ...
      doc.text("PURCHASE REQUEST", headerTextX, headerTextY + 15, {
        align: "center",
      });
    }
    ...
    doc.text(
      `Purchase Request #${request?.requestNumber?.replace("PR-", "") || "12345"}`,
      margin + 5,
      detailsBoxY + 6,
    );
    ...
    doc.setFont(request?.pdfSettings?.fontFamily || "helvetica", "normal");
    doc.text(
      `${request?.requester?.username || "John Smith"}`,
      margin + 35,
      detailsBoxY + 14,
    );
    ...
    doc.setFont(request?.pdfSettings?.fontFamily || "helvetica", "normal");
    doc.text(
      `${request?.requester?.department || "Engineering"}`,
      pageWidth / 2 + 35,
      detailsBoxY + 14,
    );
  ...
  try {
    ...
    if (request?.pdfSettings?.footerHeight) {
      ...
      footerHeight = Math.min(
        Math.max(request.pdfSettings.footerHeight / 2, 10),
        100,
      );
    }
    ...
    if (!settings.pageNumberPosition) {
      ...
      console.log(
        "Using specified page number position:",
    ...
    if (request?.pdfSettings) {
      ...
      settings = {
        ...
        pageNumberPosition:
          request.pdfSettings.pageNumberPosition || settings.pageNumberPosition,
        // Fix visibility issues by checking for explicit boolean values
      ...
      try {
        ...
        if (response.ok) {
          ...
          settings = {
            ...
            pageNumberPosition:
              apiSettings.pageNumberPosition || settings.pageNumberPosition,
            // Fix visibility issues by checking for explicit boolean values
    ...
    doc.setFillColor(accentColor[0], accentColor[1], accentColor[2]);
    doc.roundedRect(
      margin + (2 * pageWidth) / 3,
      footerY + 2,
      pageWidth / 3 - margin - 1,
      2,
      1,
      1,
      "F",
    ...
    if (settings.showFooterText === true || settings.pageNumbering === true) {
      ...
      if (settings.pageNumbering === true) {
        ...
        const pageNumberPosition =
          request?.pdfSettings?.pageNumberPosition ||
          settings.pageNumberPosition ||
          "bottom-right";
        ...
        switch (pageNumberPosition) {
          ...
          case "bottom-center":
            ...
            doc.text(pageText, pageWidth / 2, pageHeight - 4, {
              align: "center",
            });
            break;
          case "bottom-right":
          default:
            ...
            doc.text(pageText, pageWidth - margin - 5, pageHeight - 4, {
              align: "right",
            });
            break;
        }
  ...
  try {
    ...
    if (request?.pdfSettings) {
      ...
      if (request.pdfSettings.hasOwnProperty("showBasicInfo"))
        templateConfig.showBasicInfo = Boolean(
          request.pdfSettings.showBasicInfo,
        );
      if (request.pdfSettings.hasOwnProperty("showRequesterDetails"))
        templateConfig.showRequesterDetails = Boolean(
          request.pdfSettings.showRequesterDetails,
        );
      if (request.pdfSettings.hasOwnProperty("showDateOfRequest"))
        templateConfig.showDateOfRequest = Boolean(
          request.pdfSettings.showDateOfRequest,
        );
      if (request.pdfSettings.hasOwnProperty("showPurposeInfo"))
        templateConfig.showPurposeInfo = Boolean(
          request.pdfSettings.showPurposeInfo,
        );
      if (request.pdfSettings.hasOwnProperty("showVendorDetails"))
        templateConfig.showVendorDetails = Boolean(
          request.pdfSettings.showVendorDetails,
        );
      ...
      if (request.pdfSettings.hasOwnProperty("showApprovals"))
        templateConfig.showApprovals = Boolean(
          request.pdfSettings.showApprovals,
        );
      if (request.pdfSettings.hasOwnProperty("showAttachments"))
        templateConfig.showAttachments = Boolean(
          request.pdfSettings.showAttachments,
        );
      if (request.pdfSettings.hasOwnProperty("showAuditInfo"))
        templateConfig.showAuditInfo = Boolean(
          request.pdfSettings.showAuditInfo,
        );
      if (request.pdfSettings.hasOwnProperty("showSignatures"))
        templateConfig.showSignatures = Boolean(
          request.pdfSettings.showSignatures,
        );
      ...
      if (
        request.pdfSettings.marginRight ||
        request.pdfSettings.marginRight === 0
      )
        templateConfig.margins.right = request.pdfSettings.marginRight;
      if (
        request.pdfSettings.marginBottom ||
        request.pdfSettings.marginBottom === 0
      )
        templateConfig.margins.bottom = request.pdfSettings.marginBottom;
      if (
        request.pdfSettings.marginLeft ||
        request.pdfSettings.marginLeft === 0
      )
    ...
    if (templateConfig.showItems === true) {
      ...
      const itemsTotal = items.reduce(
        (sum: number, item: any) =>
          sum + Number(item?.quantity || 0) * Number(item?.estimatedCost || 0),
        0,
      );
    ...
    // Approvals Section
    if (
      templateConfig.showApprovals === true &&
      (roleForAudit === "approver" ||
        roleForAudit === "admin" ||
      ...
      if (approvals.length > 0) {
        ...
        const approvalRows = approvals.map((approval: any) => [
          ...
          approval.processedAt
            ? new Date(approval.processedAt).toLocaleString()
            : "Not processed",
          approval.comments || "",
        ]);
    ...
    if (
      templateConfig.showAuditInfo === true &&
      (roleForAudit === "admin" || request.pdfSettings?.showAuditInfo === true)
    ) {
